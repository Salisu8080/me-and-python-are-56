"""
pip install requests beautifulsoup4 tqdm pdfplumber python-docx re pandas
"""
...
def generate_keywords_from_title(title):
    """Generate keywords from title if no keywords are found in the document."""
    # Common stop words to exclude
    stop_words = set([
        'a', 'an', 'the', 'and', 'or', 'but', 'of', 'on', 'in', 'to', 'for', 'with',
        'by', 'at', 'from', 'as', 'is', 'are', 'was', 'were', 'be', 'been', 'being',
        'have', 'has', 'had', 'do', 'does', 'did', 'can', 'could', 'will', 'would',
        'shall', 'should', 'may', 'might', 'must', 'that', 'which', 'who', 'whom',
        'this', 'these', 'those', 'what', 'when', 'where', 'why', 'how'
    ])
    
    # Normalize and split the title
    title = title.lower()
    ...
    # Limit to 4 keywords, prioritizing longer words
...
def clean_author_names(authors):
    """Clean author names by removing numbers and asterisks."""
    # Remove numbers and asterisks
    cleaned = re.sub(r'[0-9*]', '', authors)
...
def convert_to_title_case(text):
    """Convert text to proper title case following standard title capitalization rules."""
    # Skip conversion if text is empty or only contains one character
    if not text or len(text) <= 1:
        return text
    
    # Small words that should be lowercase in titles (unless they're first or last)
    small_words = {'a', 'an', 'the', 'and', 'but', 'or', 'for', 'nor', 'on', 'at', 
                 'to', 'from', 'by', 'in', 'of', 'as', 'with', 'into'}
    
    # Handle hyphenated words
    def process_hyphenated_word(hyphenated):
        parts = hyphenated.split('-')
        processed_parts = []
        for i, part in enumerate(parts):
            if part.lower() in small_words and i != 0:
                processed_parts.append(part.lower())
            else:
                processed_parts.append(part.capitalize())
        return '-'.join(processed_parts)
    
    # Always convert text to lowercase first for consistent processing
    # (we'll handle proper nouns and acronyms later)
    text = text.lower()
    ...
    for i in range(len(words)):
        ...
        # Handle hyphenated words
        if '-' in word:
            words[i] = process_hyphenated_word(word)
            continue
        
        # Lowercase small words unless they're the first or last word
        if word in small_words and i != 0 and i != len(words) - 1:
            words[i] = word
        else:
    ...
    # Handle common proper nouns and abbreviations
    proper_nouns = {
        'nigeria': 'Nigeria', 'africa': 'Africa', 'asia': 'Asia', 'europe': 'Europe', 
        'america': 'America', 'australia': 'Australia', 'niger': 'Niger', 'lagos': 'Lagos', 
        'abuja': 'Abuja', 'usa': 'USA', 'uk': 'UK', 'eu': 'EU', 'un': 'UN', 'who': 'WHO', 
        'ph': 'pH', 'phd': 'PhD', 'ict': 'ICT', 'soil': 'Soil', 'soils': 'Soils',
        'kaduna': 'Kaduna', 'kano': 'Kano', 'zaria': 'Zaria', 'njss': 'NJSS'
    }
    
    # Apply proper case for specific terms
    ...
    for numeral in roman_numerals:
        result = re.sub(r'\b' + numeral + r'\b', numeral.upper(), result, flags=re.IGNORECASE)
    
    return result


def process_file_for_metadata(file_path, title=""):
    ...
    try:
        ...
        if ext == '.pdf':
            ...
            return {"email": "info@njss.org.ng", "keywords": ""}
        
        # Extract emails and keywords
        emails = extract_emails_from_text(text)
        keywords = extract_keywords_from_text(text)
        
        # Set default email if none found
        email_str = ", ".join(emails) if emails else "info@njss.org.ng"
        
        # Generate keywords from title if none found
        ...
        return {
            "email": "info@njss.org.ng",
            "keywords": ""
        }


def process_volume(url, folder, max_workers=8):
    """Process a single journal volume with the given URL and folder name."""
    ...
    try:
        ...
        print(f"Connection Error for {url}: Failed to establish a connection to the website.")
        return
    except requests.exceptions.Timeout:
        print(f"Timeout Error for {url}: The request timed out. Try again later.")
        return
    ...
    if len(all_topics) < 1:
        print(f"Error: No topics found for {url}. Check the website structure.")
        return
    ...
    print(f"Processing {num_articles} articles for {folder}...")
    ...
    for i in range(num_articles):
        ...
        topic = convert_to_title_case(topic)
        # Clean author names (remove numbers, asterisks)
        ...
        article_data = {
            ...
            "file_path": filename,            # Store just the filename, not full path
    ...
    if download_tasks:
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
            ...
            for future in tqdm(concurrent.futures.as_completed(future_to_article), 
                              ...
                              desc=f"Downloading articles for {folder}"):
                ...
                try:
                    ...
                    if result['success']:
                        ...
                        metadata[idx]['file_path'] = os.path.basename(actual_path)
    ...
    if successful_downloads:
        ...
        for article_data, file_path, idx in tqdm(successful_downloads, desc=f"Processing files for {folder}"):
            try:
                file_ext = os.path.splitext(file_path)[1].lower()
                print(f"Extracting metadata from: {os.path.basename(file_path)} ({file_ext})")
                
                # Process file based on its extension with title for backup keywords
                file_metadata = process_file_for_metadata(file_path, article_data["title"])
                ...
                # Ensure we have keywords - if still empty, generate from title
                if not metadata[idx]["keywords"]:
                    title_keywords = generate_keywords_from_title(article_data["title"])
                    metadata[idx]["keywords"] = ", ".join(title_keywords)
                
                print(f"  Keywords: {metadata[idx]['keywords'][:100]}...")
                print(f"  Email: {metadata[idx]['email']}")
            except Exception as e:
                print(f"Error processing file {file_path}: {str(e)}")
                # Set defaults in case of error
                metadata[idx]["email"] = "info@njss.org.ng"
                title_keywords = generate_keywords_from_title(article_data["title"])
                metadata[idx]["keywords"] = ", ".join(title_keywords)
    ...
    print(f"\nComplete! Processed {len(metadata)} articles for {folder}.")
...
def process_all_volumes(csv_file_path):
    """Process all journal volumes from the CSV file."""
    try:
        # Read the CSV file
        df = pd.read_csv(csv_file_path)
        print(f"Loaded {len(df)} volumes from CSV file")
        
        # Check if the CSV has the required columns
        if 'url' not in df.columns or 'folder' not in df.columns:
            print("Error: CSV file must have 'url' and 'folder' columns")
            return
        ...
        for index, row in df.iterrows():
            ...
            print(f"\n{'='*80}")
            print(f"Processing volume {index+1}/{len(df)}: {folder} from {url}")
            print(f"{'='*80}\n")
            
            # Process this volume
            process_volume(url, folder)
            
        print(f"\nAll {len(df)} volumes processed successfully!")
...
def main():
    """Main function to process all volumes from the CSV file."""
    # Look for the CSV file in the current directory
    csv_file_path = "soilsjournal_volumes.csv"
    
    # Check if the CSV file exists
    if not os.path.exists(csv_file_path):
        # If not in current directory, check in the Downloads folder
        downloads_dir = str(Path.home() / "Downloads")
        csv_file_path = os.path.join(downloads_dir, "soilsjournal_volumes.csv")
        
        if not os.path.exists(csv_file_path):
            print(f"Error: Could not find soilsjournal_volumes.csv in the current directory or Downloads folder")
            return
    
    print(f"Found CSV file: {csv_file_path}")
    process_all_volumes(csv_file_path)
